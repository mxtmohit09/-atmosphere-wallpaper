  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (i === 4) {
      ...
      if (i === 0) {
        ...
        opacity = 0.4 + Math.random() * 0.3; // 40-70% opacity for bright blobs
      } else {
    ...
    const drawElongatedBlob = (centerX: number, centerY: number, targetSize: number) => {
      ...
      if (i === 4) {
        ...
        horizontalStretch = 1 + Math.random() * 2 * elongationFactor; // 1-3x horizontal stretch
        verticalStretch = 0.3 + Math.random() * 0.4; // 0.3-0.7x vertical stretch
      }
      
      // Pre-calculate random variations for better performance
      const radiusVariations = Array.from({length: numPoints}, () => 0.8 + Math.random() * 0.4); // Reduced variation for smoother shapes
      const offsetsX = Array.from({length: numPoints}, () => (Math.random() - 0.5) * baseRadius * 0.15); // Reduced offset for smoother shapes
      const offsetsY = Array.from({length: numPoints}, () => (Math.random() - 0.5) * baseRadius * 0.15);
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        const controlOffset = baseRadius * (0.05 + Math.random() * 0.15); // Reduced randomness for smoother curves
